...
interface TestimonialOpportunity {
  available: boolean;
  clientName?: string;
  message?: string;
}
...
interface IProps {
  opportunity: TestimonialOpportunity;
}

export const TestimonialOpportunity = ({ opportunity }: IProps) => {
  return (
      ...
      <div className="relative z-10">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="text-stone-50 text-md font-medium leading-relaxed">
            {opportunity.available ? 'New Testimonial Opportunity' : 'Testimonial Opportunities'}
          </h3>
        </div>
        ...
        {opportunity.available ? (
          <>
            <div className="mb-6">
              <p className="text-stone-300 text-sm mb-4">
                {opportunity.clientName}: "{opportunity.message}"
              </p>
            </div>
          ...
          <div className="text-center py-8">
            <div className="mb-4">
              <Heart className="w-12 h-12 text-stone-500 mx-auto" />
            </div>
            <p className="text-stone-400 text-sm leading-relaxed">
              No testimonials yet. Start engaging with clients to collect your first testimonial!
            </p>
            <p className="text-stone-500 text-xs mt-2">
              Great client relationships lead to powerful testimonials.
            </p>
          </div>
        )}
      </div>
