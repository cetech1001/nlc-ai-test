        ...
        <div className="flex gap-1">
          ...
          <button
            ...
            className={`px-2 py-1 text-xs rounded transition-colors ${
              isOwner
                ? 'bg-purple-600/10 text-purple-400/50 border border-purple-600/20 cursor-not-allowed'
          ...
          {member.status === 'active' ? (
            <button
              ...
              className={`px-2 py-1 text-xs rounded transition-colors ${
                isOwner
                  ? 'bg-red-600/10 text-red-400/50 border border-red-600/20 cursor-not-allowed'
            ...
            <button
              ...
              className="px-2 py-1 text-xs bg-green-600/20 hover:bg-green-600/30 border border-green-600/30 rounded text-green-400 transition-colors"
            >
              Activate
            </button>
          )}
          <button
            onClick={() => onRowAction?.('delete', member)}
            disabled={isOwner}
            className={`px-2 py-1 text-xs rounded transition-colors ${
              isOwner
                ? 'bg-red-600/10 text-red-400/50 border border-red-600/20 cursor-not-allowed'
              ...
              <div className="relative z-10">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <div className="text-white">{member._count.posts} posts • {member._count.comments} comments</div>
                  </div>
                  <div>
                ...
                {onRowAction && (
                  <div className="flex flex-col gap-2 pt-2 border-t border-neutral-700">
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        onClick={() => onRowAction('view-profile', member)}
                        className="px-3 py-2 bg-neutral-700 hover:bg-neutral-600 rounded-lg text-white text-sm transition-colors"
                      >
                        View Profile
                      </button>
                      <button
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {member.status === 'active' ? (
                        ...
                        <button
                          onClick={() => onRowAction('activate', member)}
                          className="px-3 py-2 bg-green-600/20 hover:bg-green-600/30 border border-green-600/30 rounded-lg text-green-400 text-sm transition-colors"
                        >
                          Activate
                        </button>
                      )}
