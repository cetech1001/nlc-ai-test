...
export const useMessagingWebSocket = (options: UseMessagingWebSocketOptions = {}) => {
  ...
  }, [options.user?.id, options.user?.type, options.onUserTyping]);
  ...
  const sendTypingStatus = useCallback((conversationID: string, isTyping: boolean) => {
    ...
    const timeoutKey = `${conversationID}:${options.user?.id}`;
  ...
  useEffect(() => {
    if (!options.enabled || !token || !options.user) {
      return;
    }
